...
const SuspendDHCPClientModal = ({ closeSuspendDHCPClientModal, serviceID }) => {
    const [showSuccessModal, setShowSuccessModal] = useState(false);
    const { handleError, ErrorModalComponent } = useErrorHandler();

    const handleSuspendClick = async () => {
        try {
            const res = await api.delete(`/api/clients/dhcp/client/suspend/${serviceID}/`);
            if (res.status === 204 ) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center">
            <div className="fixed inset-0 bg-gray-900 bg-opacity-50" onClick={closeSuspendDHCPClientModal}></div>
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg mt-10 transform transition-transform duration-300">
                <div className="flex items-center mb-4">
                    <FontAwesomeIcon
                        ...
                        onClick={closeSuspendDHCPClientModal}
                        title="Close"
                    />
                    <h2 className="text-xl flex-1 font-semibold text-blue-900 text-center">Suspend service</h2>
                </div>
                <div className="mb-4">
                    <p className="text-gray-700 text-center">Are you sure you want to suspend this service?</p>
                </div>
...
export default SuspendDHCPClientModal;
