...
const DeleteUnifiModal = ({ closeDeleteUnifiModal, data }) => {
    const [showSuccessModal, setShowSuccessModal] = useState(false);
    const navigate = useNavigate();

    const handleDeleteClick = async () => {
        try {
          const response = await api.delete(`/api/hotspot/credentials/${data.id}/`);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center">
            <div className="fixed inset-0 bg-gray-900 bg-opacity-50" onClick={closeDeleteUnifiModal}></div>
...
export default DeleteUnifiModal;
