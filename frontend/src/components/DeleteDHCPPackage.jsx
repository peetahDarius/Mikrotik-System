...
const DeleteDHCPPackageModal = ({ closeDeleteDHCPPackageModal, dhcpPackage }) => {
    const [showSuccessModal, setShowSuccessModal] = useState(false);
    const navigate = useNavigate();
    const { handleError, ErrorModalComponent } = useErrorHandler();
    const handleDeleteClick = async () => {
        try {
          const response = await api.delete(`/api/static/packages/${dhcpPackage.id}/`);
          ...
          navigate('/dhcp/packages');
        }
    };

    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center">
            <div className="fixed inset-0 bg-gray-900 bg-opacity-50" onClick={closeDeleteDHCPPackageModal}></div>
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg mt-10 transform transition-transform duration-300">
                <div className="flex items-center mb-4">
                    <FontAwesomeIcon
                        ...
                        onClick={closeDeleteDHCPPackageModal}
                        title="Close"
                    />
...
export default DeleteDHCPPackageModal;
