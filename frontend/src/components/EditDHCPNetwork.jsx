...
const EditDHCPNetwork = ({ closeEditDHCPNetworkModal, network }) => {
    const [address, setAddress] = useState(network.address);
    const [gateway, setGateway] = useState(network.gateway);
    const [dnsServer, setDnsServer] = useState(network.primary_dns);
    const [secondaryDns, setSecondaryDns] = useState(network.secondary_dns);
    const [defaultLimit, setDefaultLimit] = useState(network.default_limit)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.put(`/api/dhcp/networks/${network.id}/`, JSON.stringify(data));
      if (response.status === 200) {
        closeEditDHCPNetworkModal();
      }
    } catch (error) {
      handleError(error);
    }
    ...
    <div className="fixed inset-0 flex items-center justify-end bg-gray-900 bg-opacity-50 z-50">
      <div className="w-full p-8 space-y-6 bg-white shadow-md rounded-lg max-w-sm h-screen">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-center text-indigo-600">Edit DHCP Network</h2>
          <button
            onClick={closeEditDHCPNetworkModal}
...
export default EditDHCPNetwork;
