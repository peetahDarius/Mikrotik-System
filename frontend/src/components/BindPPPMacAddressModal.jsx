...
const BindPPPMacAddressModal = ({ closeBindPPPMacAddressModal, serviceID, username}) => {
  ...
  const fetchMacAddress = async () => {
    try {
      const data = {username}
      const response = await api.post("api/clients/ppp/client/mac/", data)
      setMacAddress(response.data["caller_id"])
    } catch (error) {
      console.log(error)
    }
  }
  ...
  const handleSaveClick = async () => {
    const data = { caller_id: macAddress };
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center">
      <div className="fixed inset-0 bg-gray-900 bg-opacity-50" onClick={closeBindPPPMacAddressModal}></div>
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg mt-10 transform transition-transform duration-300">
        <div className="flex items-center mb-4">
          <FontAwesomeIcon
            ...
            onClick={closeBindPPPMacAddressModal}
            title="Close"
          />
...
export default BindPPPMacAddressModal;
