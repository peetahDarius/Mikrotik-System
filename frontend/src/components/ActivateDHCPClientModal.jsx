...
const ActivateDHCPClientModal = ({ closeActivateDHCPClientModal, serviceID }) => {
    const [showSuccessModal, setShowSuccessModal] = useState(false);
    const { handleError, ErrorModalComponent } = useErrorHandler();
    const handleActivateClick = async () => {
        try {
            const res = await api.patch(`/api/clients/dhcp/client/activate/${serviceID}/`);
            if (res.status === 200 ) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center">
            <div className="fixed inset-0 bg-gray-900 bg-opacity-50" onClick={closeActivateDHCPClientModal}></div>
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg mt-10 transform transition-transform duration-300">
                <div className="flex items-center mb-4">
                    <FontAwesomeIcon
                        ...
                        onClick={closeActivateDHCPClientModal}
                        title="Close"
                    />
...
export default ActivateDHCPClientModal;
