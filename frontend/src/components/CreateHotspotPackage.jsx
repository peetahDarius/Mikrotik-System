...
const CreateHotspotPackage = ({ closeCreateHotspotPackageModal }) => {
  const [name, setName] = useState('');
  const [duration, setDuration] = useState(null);
  const [up, setUp] = useState('');
  const [down, setDown] = useState('');
  const [byteQuota, setByteQuota] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post('/api/hotspot/packages/', JSON.stringify(data));
      if (response.status === 201) {
        closeCreateHotspotPackageModal();
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 flex items-center justify-end bg-gray-900 bg-opacity-50 z-50">
      <div className="w-full p-8 space-y-6 bg-white shadow-md rounded-lg max-w-sm h-screen">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-center text-indigo-600">Add Package</h2>
          <button
            onClick={closeCreateHotspotPackageModal}
...
export default CreateHotspotPackage;
