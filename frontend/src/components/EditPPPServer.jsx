...
const EditPPPServer = ({ closeEditPPPServerModal, pppServer }) => {
    const [name, setName] = useState(pppServer.name);
    const [max_mtu, setMaxMtu] = useState(pppServer.max_mtu);
    const [max_mru, setMaxMru] = useState(pppServer.max_mtu);
    const [selectedInterface, setSelectedInterface] = useState(pppServer.interface);
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await api.put(`/api/ppp/server/${pppServer.id}/`, JSON.stringify(data));

        if (response.status === 200) {
            closeEditPPPServerModal();
        }
    };
        ...
        <div className="fixed inset-0 flex items-center justify-end bg-gray-900 bg-opacity-50 z-50">
            <div className="w-full p-8 space-y-6 bg-white shadow-md rounded-lg max-w-sm h-screen">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-center text-indigo-600">Edit PPP Server</h2>
                    <button
                        onClick={closeEditPPPServerModal}
