...
const CreateAddress = ({ closeCreateAddressModal }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        e.preventDefault();
        const response = await api.post('/api/ip/addresses/', JSON.stringify(data))
    ...
    <div className="fixed inset-0 flex items-center justify-end bg-gray-900 bg-opacity-50 z-50">
       <div className="w-full p-8 space-y-6 bg-white shadow-md rounded-lg max-w-sm h-screen">
                 <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-center text-indigo-600">Add IP Address</h2>
                 <button
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        <label htmlFor="ipAddress" className="block text-sm font-medium text-gray-700">Address/Address Range</label>
                        <input
                            type="text"
                            id="ipAddress"
                            value={ipAddress}
